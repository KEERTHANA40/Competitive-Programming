...
def inord(root_node):
    res1=traversing(root_node)
    return res1

def traversing(root_node):
    if root_node:
        inord(root_node.left)
        res.append(root_node.value)
        inord(root_node.right)
    return res

def secondLargest(element):
    if len(element)<=1:
        raise Exception
    else:
...
def find_second_largest(root_node):
    ...
    res2 = inord(root_node)
    output=secondLargest(res2)
    global res
    res[:]=[]
